...
class DiscDCGAN(nn.Module):
    def __init__(self, ndf=64):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(1, ndf, 4, 2, 1), nn.LeakyReLU(0.2, True),   # 28->14
            nn.Conv2d(ndf, ndf*2, 4, 2, 1), nn.BatchNorm2d(ndf*2), nn.LeakyReLU(0.2, True),  # 14->7
            nn.Flatten(),
            nn.Linear(ndf*2*7*7, 1), nn.Sigmoid()
        )
    def forward(self,x):
        return self.net(x)
